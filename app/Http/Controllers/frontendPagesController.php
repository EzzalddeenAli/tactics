...
class frontendPagesController extends Controller {
	...
	public function get_news_list($page){
		...
		$news_list = \newsboard::where('fe_active','1')->orderby('id','desc')->take('20')->skip(20* ($page - 1) )->get()->toArray();
		foreach ($news_list as $key => $value) {
			...
			if(!\File::exists("uploads/news/".$value['newsImage']) || $value['newsImage'] == ""){
	...
	public function get_events_list($page){
		...
		$events = \events::where('fe_active','1')->orderby('id','desc')->take('20')->skip(20* ($page - 1) )->get()->toArray();
		foreach ($events as $key => $value) {
			...
			if(!\File::exists("uploads/events/".$value['eventImage']) || $value['eventImage'] == ""){
