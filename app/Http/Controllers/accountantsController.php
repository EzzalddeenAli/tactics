...
class accountantsController extends Controller {
	...
	function account_status($id){
		if($this->data['users']->role != "admin") exit;
		$user = \User::where('role','account')->where('id',$id)->first();

		if($id == $this->data['users']->id){
			return $this->panelInit->apiOutput(false,$this->panelInit->language['editAccountant'],$this->panelInit->language['accStatusCantYourself'] );
		}
		...
		return $this->panelInit->apiOutput(true,$this->panelInit->language['editAccountant'],$this->panelInit->language['accStatusChged'], array( 'id' => $user->id,'account_active' => $user->account_active ) );
	}
}
